...
# Load gateway config from config.env or gateway_config.json
GATEWAY_ID = os.getenv('GATEWAY_ID', '')
GATEWAY_ARN = os.getenv('GATEWAY_ARN', '')
if not GATEWAY_ID:
    try:
        with open(str(ROOT_DIR / 'gateway_config.json'), 'r') as f:
            gateway_config = json.load(f)
        GATEWAY_ID = gateway_config.get('gateway_id', '')
        GATEWAY_ARN = gateway_config.get('gateway_arn', '')
    except FileNotFoundError:
        pass

# Auto-discover gateway if still not found
...
def create_policies(policy_engine_id):
  ...
  resource == AgentCore::Gateway::"{GATEWAY_ARN}"
    ...
    # ============================================================================
    # TODO 7.1: Uncomment the forbid policy below to block analysts from creating bookings
    # This Cedar policy says: "Forbid any OAuthUser whose custom:role tag equals
    # 'analyst' from calling APIInteg___create_booking_tool."
    # ============================================================================
    forbid_write_policy = None  # TODO 7.1: Uncomment the block below
...
when {{
  ...
  principal.getTag("custom:role") == "staff"
    ...
    policy_definitions = [(n, d, p) for n, d, p in policy_definitions if p is not None]
...
def main():
    ...
    print(f"\n[OK] Deployment Complete! Mode: LOG_ONLY")
    print(f"Next: python deploy_policy.py --enforce")
