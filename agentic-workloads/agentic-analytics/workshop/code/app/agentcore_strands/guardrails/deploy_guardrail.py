...
GUARDRAIL_CONFIG = dict(
    ...
    # TODO 8.1 (Step 8): Uncomment the denied topics below.
    # These define what the guardrail blocks:
    # - DangerousAdvice: medical, legal, financial advice
    # - DatabaseSchemaLeakage: raw SQL, table names, account_id — protects your SaaS internals
    #
    # Key design decisions:
    #   DangerousAdvice: outputEnabled=False (only check INPUT, not OUTPUT)
    #     -> Prevents false positives on analytics data in agent responses
    #   DatabaseSchemaLeakage: inputEnabled=False, outputEnabled=True (only check OUTPUT)
    #     -> Catches schema leaks in agent responses, not user questions
